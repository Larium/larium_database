...
use Closure;
/**
 * Paginator class from Laravel framework.
 *
...
class Paginator {
     ...
     * @return void
     */
    protected function __construct($results, $page, $total, $per_page, $last, $query, $uri)
    {
        $this->page = $page;
        $this->last = $last;
        $this->total = $total;
     ...
     * @return Paginator
     */
    public static function make($results, $total, $per_page, $query, $uri)
    {
        $page = static::page($query, $total, $per_page);

        $last = ceil($total / $per_page);

        return new static($results, $page, $total, $per_page, $last, $query, $uri);
    }
     ...
     * @return int
     */
    public static function page($query, $total, $per_page)
    {
    ...
    protected function element($element, $page, $text, $disabled)
    {
        ...
        if (is_null($text))
        {
            $text = $element; //TODO:i18n Lang::line("pagination.{$element}")->get($this->language);
        }
        ...
        {
            return "<li class=\"disabled\"><span class=\"{$class} disabled\">{$text}</spam></li>";
        }
        else
        {
            return $this->link($page, $text, $class) . "</li>";
        }
    }
    ...
    protected function range($start, $end)
    {
        ...
        for ($page = $start; $page <= $end; $page++)
        {
            if ($this->page == $page)
            {
                $pages[] = "<li class=\"active\"><span class=\"current\">{$page}</span></li>";
            }
            else
            {
    ...
    protected function link($page, $text, $class)
    {
        ...
        return "<li><a href=\"{$uri}{$query}\" class=\"{$class}\">{$text}</a></li>";
    }
